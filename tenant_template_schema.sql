...
--
-- Name: sp_client_create(bigint, character varying, character varying, character varying, character varying, character varying, text, text, date); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.sp_client_create(p_empresa_id bigint, p_name character varying, p_legal_name character varying, p_tax_id character varying, p_email character varying, p_phone character varying, p_address text, p_notes text, p_birth_date date DEFAULT NULL::date) RETURNS TABLE(id bigint, name character varying)
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
            BEGIN
                RETURN QUERY
                INSERT INTO clientes (
                    empresa_id, name, legal_name, tax_id,
                    email, phone, address, notes, birth_date
                )
                VALUES (
                    p_empresa_id, p_name, p_legal_name, p_tax_id,
                    p_email, p_phone, p_address, p_notes, p_birth_date
                )
                RETURNING clientes.id, clientes.name;
            END; $$;
...
CREATE FUNCTION public.sp_client_find(p_id bigint) RETURNS TABLE(id bigint, empresa_id bigint, name character varying, legal_name character varying, tax_id character varying, email character varying, phone character varying, address text, notes text, birth_date date, is_active boolean, created_at timestamp without time zone, updated_at timestamp without time zone)
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
            BEGIN
                RETURN QUERY
...
CREATE FUNCTION public.sp_client_list(p_empresa_id bigint) RETURNS TABLE(id bigint, empresa_id bigint, name character varying, legal_name character varying, tax_id character varying, email character varying, phone character varying, address text, notes text, birth_date date, is_active boolean, created_at timestamp without time zone, updated_at timestamp without time zone)
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
            BEGIN
                RETURN QUERY
...
CREATE FUNCTION public.sp_client_search(p_empresa_id bigint, p_query character varying) RETURNS TABLE(id bigint, empresa_id bigint, name character varying, legal_name character varying, tax_id character varying, email character varying, phone character varying, address text, notes text, birth_date date, is_active boolean, created_at timestamp without time zone, updated_at timestamp without time zone)
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
            BEGIN
                RETURN QUERY
                ...
                WHERE c.empresa_id = p_empresa_id
                  ...
                  AND (
                      ...
                      c.legal_name ILIKE '%' || p_query || '%' OR
                      c.tax_id     ILIKE '%' || p_query || '%' OR
...
--
-- Name: sp_client_update(bigint, character varying, character varying, character varying, character varying, character varying, text, text, date); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.sp_client_update(p_id bigint, p_name character varying, p_legal_name character varying, p_tax_id character varying, p_email character varying, p_phone character varying, p_address text, p_notes text, p_birth_date date DEFAULT NULL::date) RETURNS boolean
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
            DECLARE v_rows INTEGER;
            BEGIN
...
CREATE FUNCTION public.sp_consecutivo_next(p_empresa_id bigint, p_sucursal_id bigint, p_sucursal_codigo character varying, p_terminal character varying, p_tipo_documento character varying, p_situacion smallint DEFAULT 1) RETURNS TABLE(consecutivo bigint, numero_consecutivo character varying)
    ...
    AS $$
            ...
            BEGIN
                INSERT INTO documento_consecutivos (empresa_id, sucursal_id, tipo_documento, ultimo_consecutivo)
                VALUES (p_empresa_id, p_sucursal_id, p_tipo_documento, 1)
                ...
                -- Formato: SUCURSAL(3) + TERMINAL(5) + TIPO(2) + CONSECUTIVO(10) = 20 chars
...
BEGIN
    INSERT INTO documentos_electronicos (
        ...
        receptor_tipo_id, receptor_numero_id, receptor_nombre_comercial,
        ...
        total_imp_asumido_emisor, total_iva_devuelto, total_otros_cargos, total_comprobante,
        otros_texto, otros_contenido, funcionario_id, estado
    ) VALUES (
        ...
        p_receptor_tipo_id, p_receptor_numero_id, p_receptor_nombre_comercial,
        ...
        p_total_imp_asumido_emisor, p_total_iva_devuelto, p_total_otros_cargos, p_total_comprobante,
        p_otros_texto, p_otros_contenido, p_funcionario_id, 'borrador'
    ...
    AS $$
            BEGIN
                ...
                SELECT
                    ...
                    -- Líneas con sub-arrays embebidos
                    COALESCE((
                        SELECT jsonb_agg(
                            jsonb_build_object(
                                'id', l.id,
...
CREATE FUNCTION public.sp_empresa_hacienda_get(p_empresa_id bigint) RETURNS SETOF public.empresa_hacienda_config
    ...
    AS $$
BEGIN
    RETURN QUERY SELECT * FROM empresa_hacienda_config
...
CREATE FUNCTION public.sp_factura_recibida_create(p_empresa_id bigint, p_clave character varying, p_tipo_documento character varying, p_numero_consecutivo character varying, p_fecha_emision timestamp without time zone, p_emisor_tipo_id character varying, p_emisor_numero_id character varying, p_emisor_nombre character varying, p_receptor_tipo_id character varying, p_receptor_numero_id character varying, p_receptor_nombre character varying, p_moneda character varying, p_total_comprobante numeric, p_total_impuesto numeric, p_xml_recibido text, p_estado_hacienda character varying, p_hacienda_mensaje text) RETURNS bigint
    LANGUAGE plpgsql
    AS $$                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              DECLARE v_id BIGINT;                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           BEGIN                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          INSERT INTO facturas_recibidas(                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            empresa_id, clave, tipo_documento, numero_consecutivo_emisor,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          fecha_emision, emisor_tipo_id, emisor_numero_id, emisor_nombre,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        receptor_tipo_id, receptor_numero_id, receptor_nombre,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 moneda, total_comprobante, total_impuesto,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             xml_recibido, estado_hacienda, hacienda_mensaje                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        )                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          VALUES(                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    p_empresa_id, p_clave, p_tipo_documento, p_numero_consecutivo,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         p_fecha_emision, p_emisor_tipo_id, p_emisor_numero_id, p_emisor_nombre,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                p_receptor_tipo_id, p_receptor_numero_id, p_receptor_nombre,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           p_moneda, p_total_comprobante, p_total_impuesto,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       p_xml_recibido, p_estado_hacienda, p_hacienda_mensaje                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  )                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          ON CONFLICT (empresa_id, clave)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            DO UPDATE SET                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              estado_hacienda  = EXCLUDED.estado_hacienda,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           hacienda_mensaje = EXCLUDED.hacienda_mensaje,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          updated_at       = NOW()                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               RETURNING id INTO v_id;                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    RETURN v_id;                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               END;                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           $$;
...
CREATE FUNCTION public.sp_factura_recibida_get(p_id bigint, p_empresa_id bigint) RETURNS SETOF public.facturas_recibidas
    LANGUAGE plpgsql
    AS $$                                                                              BEGIN                                                                          RETURN QUERY                                                               SELECT * FROM facturas_recibidas                                           WHERE id = p_id AND empresa_id = p_empresa_id;                             END;                                                                           $$;
...
CREATE FUNCTION public.sp_factura_recibida_list(p_empresa_id bigint, p_estado character varying DEFAULT NULL::character varying, p_search text DEFAULT NULL::text) RETURNS TABLE(id bigint, empresa_id bigint, clave character varying, tipo_documento character varying, numero_consecutivo_emisor character varying, fecha_emision timestamp without time zone, emisor_tipo_id character varying, emisor_numero_id character varying, emisor_nombre character varying, moneda character varying, total_comprobante numeric, total_impuesto numeric, estado_hacienda character varying, estado_recepcion character varying, mensaje_tipo smallint, respondido_at timestamp without time zone, created_at timestamp without time zone)
    LANGUAGE plpgsql
    AS $$                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       BEGIN                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   RETURN QUERY                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        SELECT f.id, f.empresa_id,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          f.clave, f.tipo_documento,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   f.numero_consecutivo_emisor,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 f.fecha_emision,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             f.emisor_tipo_id, f.emisor_numero_id, f.emisor_nombre,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       f.moneda, f.total_comprobante, f.total_impuesto,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             f.estado_hacienda, f.estado_recepcion,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       f.mensaje_tipo, f.respondido_at,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             f.created_at                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 FROM facturas_recibidas f                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           WHERE f.empresa_id = p_empresa_id                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   AND (p_estado IS NULL OR f.estado_recepcion = p_estado)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           AND (                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             p_search IS NULL OR p_search = ''                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               OR f.emisor_nombre   ILIKE '%' || p_search || '%'                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               OR f.emisor_numero_id ILIKE '%' || p_search || '%'                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              OR f.clave           ILIKE '%' || p_search || '%'                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               )                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 ORDER BY f.created_at DESC;                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         END;                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    $$;
...
CREATE FUNCTION public.sp_factura_recibida_responder(p_id bigint, p_empresa_id bigint, p_mensaje_tipo smallint, p_detalle_mensaje character varying, p_monto_impuesto_acreditar numeric, p_monto_total_linea_detalle numeric, p_numero_consecutivo_receptor character varying, p_xml_mensaje_receptor text, p_hacienda_respuesta_mr text, p_estado_recepcion character varying) RETURNS boolean
    LANGUAGE plpgsql
    AS $$                                                                                                                                                                                                                                                                                                                                                                             BEGIN                                                                                                                                                                                                                                                                                                                                                                         UPDATE facturas_recibidas                                                                                                                                                                                                                                                                                                                                                 SET mensaje_tipo                  = p_mensaje_tipo,                                                                                                                                                                                                                                                                                                                       detalle_mensaje               = p_detalle_mensaje,                                                                                                                                                                                                                                                                                                                    monto_impuesto_acreditar      = p_monto_impuesto_acreditar,                                                                                                                                                                                                                                                                                                           monto_total_linea_detalle     = p_monto_total_linea_detalle,                                                                                                                                                                                                                                                                                                          numero_consecutivo_receptor   = p_numero_consecutivo_receptor,                                                                                                                                                                                                                                                                                                        xml_mensaje_receptor          = p_xml_mensaje_receptor,                                                                                                                                                                                                                                                                                                               hacienda_respuesta_mr         = p_hacienda_respuesta_mr,                                                                                                                                                                                                                                                                                                              estado_recepcion              = p_estado_recepcion,                                                                                                                                                                                                                                                                                                                   respondido_at                 = NOW(),                                                                                                                                                                                                                                                                                                                                updated_at                    = NOW()                                                                                                                                                                                                                                                                                                                                 WHERE id = p_id AND empresa_id = p_empresa_id;                                                                                                                                                                                                                                                                                                                            RETURN FOUND;                                                                                                                                                                                                                                                                                                                                                             END;                                                                                                                                                                                                                                                                                                                                                                          $$;
...
CREATE FUNCTION public.sp_medio_pago_create(p_empresa_id bigint, p_nombre character varying, p_tipo_hacienda_codigo character varying, p_tipo_hacienda_nombre character varying) RETURNS bigint
    LANGUAGE plpgsql
    AS $$                                                                                                                                                                              DECLARE v_id BIGINT;                                                                                                                                                           BEGIN                                                                                                                                                                          INSERT INTO empresa_medios_pago(empresa_id, nombre, tipo_hacienda_codigo, tipo_hacienda_nombre)                                                                            VALUES (p_empresa_id, p_nombre, p_tipo_hacienda_codigo, p_tipo_hacienda_nombre)                                                                                            RETURNING id INTO v_id;                                                                                                                                                    RETURN v_id;                                                                                                                                                               END;                                                                                                                                                                           $$;
...
CREATE FUNCTION public.sp_medio_pago_delete(p_id bigint, p_empresa_id bigint) RETURNS boolean
    LANGUAGE plpgsql
    AS $$                                                                           BEGIN                                                                       UPDATE empresa_medios_pago                                              SET deleted_at = NOW(), updated_at = NOW()                              WHERE id = p_id AND empresa_id = p_empresa_id AND deleted_at IS NULL;   RETURN FOUND;                                                           END;                                                                        $$;
...
CREATE FUNCTION public.sp_medio_pago_update(p_id bigint, p_empresa_id bigint, p_nombre character varying, p_tipo_hacienda_codigo character varying, p_tipo_hacienda_nombre character varying, p_is_active boolean) RETURNS boolean
    LANGUAGE plpgsql
    AS $$                                                                                                                                                                                                                BEGIN                                                                                                                                                                                                            UPDATE empresa_medios_pago                                                                                                                                                                                   SET nombre               = p_nombre,                                                                                                                                                                         tipo_hacienda_codigo = p_tipo_hacienda_codigo,                                                                                                                                                           tipo_hacienda_nombre = p_tipo_hacienda_nombre,                                                                                                                                                           is_active            = p_is_active,                                                                                                                                                                      updated_at           = NOW()                                                                                                                                                                             WHERE id = p_id AND empresa_id = p_empresa_id AND deleted_at IS NULL;                                                                                                                                        RETURN FOUND;                                                                                                                                                                                                END;                                                                                                                                                                                                             $$;
...
CREATE FUNCTION public.sp_medios_pago_list(p_empresa_id bigint) RETURNS TABLE(id bigint, empresa_id bigint, nombre character varying, tipo_hacienda_codigo character varying, tipo_hacienda_nombre character varying, is_active boolean, created_at timestamp without time zone, updated_at timestamp without time zone)
    LANGUAGE plpgsql
    AS $$                                                                                                                                                                                                                                            BEGIN                                                                                                                                                                                                                                        RETURN QUERY                                                                                                                                                                                                                             SELECT m.id, m.empresa_id, m.nombre, m.tipo_hacienda_codigo,                                                                                                                                                                             m.tipo_hacienda_nombre, m.is_active, m.created_at, m.updated_at                                                                                                                                                                   FROM empresa_medios_pago m                                                                                                                                                                                                               WHERE m.empresa_id = p_empresa_id                                                                                                                                                                                                        AND m.deleted_at IS NULL                                                                                                                                                                                                               ORDER BY m.nombre;                                                                                                                                                                                                                       END;                                                                                                                                                                                                                                         $$;
...
CREATE FUNCTION public.sp_proveedor_create(p_empresa_id bigint, p_name character varying, p_legal_name character varying, p_tipo_id character varying, p_tax_id character varying, p_email character varying, p_phone character varying, p_address text, p_notes text) RETURNS bigint
    LANGUAGE plpgsql
    AS $$                                                                                                                                                                                                                                                                    DECLARE v_id BIGINT;                                                                                                                                                                                                                                                 BEGIN                                                                                                                                                                                                                                                                INSERT INTO proveedores(empresa_id, name, legal_name, tipo_id, tax_id, email, phone, address, notes)                                                                                                                                                             VALUES (p_empresa_id, p_name, p_legal_name, p_tipo_id, p_tax_id, p_email, p_phone, p_address, p_notes)                                                                                                                                                           RETURNING id INTO v_id;                                                                                                                                                                                                                                          RETURN v_id;                                                                                                                                                                                                                                                     END;                                                                                                                                                                                                                                                                 $$;
...
CREATE FUNCTION public.sp_proveedor_list(p_empresa_id bigint, p_search text DEFAULT NULL::text) RETURNS TABLE(id bigint, empresa_id bigint, name character varying, legal_name character varying, tipo_id character varying, tax_id character varying, email character varying, phone character varying, address text, notes text, is_active boolean, created_at timestamp without time zone, updated_at timestamp without time zone)
    LANGUAGE plpgsql
    AS $$                                                                                                                                                                                                                                                                                                                         BEGIN                                                                                                                                                                                                                                                                                                                     RETURN QUERY                                                                                                                                                                                                                                                                                                          SELECT p.id, p.empresa_id, p.name, p.legal_name, p.tipo_id, p.tax_id,                                                                                                                                                                                                                                                 p.email, p.phone, p.address, p.notes, p.is_active,                                                                                                                                                                                                                                                             p.created_at, p.updated_at                                                                                                                                                                                                                                                                                     FROM proveedores p                                                                                                                                                                                                                                                                                                    WHERE p.empresa_id = p_empresa_id                                                                                                                                                                                                                                                                                     AND p.deleted_at IS NULL                                                                                                                                                                                                                                                                                            AND (                                                                                                                                                                                                                                                                                                               p_search IS NULL OR p_search = ''                                                                                                                                                                                                                                                                                 OR p.name      ILIKE '%' || p_search || '%'                                                                                                                                                                                                                                                                       OR p.legal_name ILIKE '%' || p_search || '%'                                                                                                                                                                                                                                                                      OR p.tax_id    ILIKE '%' || p_search || '%'                                                                                                                                                                                                                                                                       OR p.email     ILIKE '%' || p_search || '%'                                                                                                                                                                                                                                                                       )                                                                                                                                                                                                                                                                                                                   ORDER BY p.name;                                                                                                                                                                                                                                                                                                      END;                                                                                                                                                                                                                                                                                                                      $$;
...
CREATE FUNCTION public.sp_proveedor_soft_delete(p_id bigint, p_empresa_id bigint) RETURNS boolean
    LANGUAGE plpgsql
    AS $$                                                                               BEGIN                                                                           UPDATE proveedores                                                          SET deleted_at = NOW(), updated_at = NOW()                                  WHERE id = p_id AND empresa_id = p_empresa_id AND deleted_at IS NULL;       RETURN FOUND;                                                               END;                                                                            $$;
...
CREATE FUNCTION public.sp_proveedor_toggle(p_id bigint, p_empresa_id bigint) RETURNS boolean
    LANGUAGE plpgsql
    AS $$                                                                          BEGIN                                                                      UPDATE proveedores                                                     SET is_active = NOT is_active, updated_at = NOW()                      WHERE id = p_id AND empresa_id = p_empresa_id AND deleted_at IS NULL;  RETURN FOUND;                                                          END;                                                                       $$;
...
CREATE FUNCTION public.sp_proveedor_update(p_id bigint, p_empresa_id bigint, p_name character varying, p_legal_name character varying, p_tipo_id character varying, p_tax_id character varying, p_email character varying, p_phone character varying, p_address text, p_notes text, p_is_active boolean) RETURNS boolean
    LANGUAGE plpgsql
    AS $$                                                                                                                                                                                                                                                                                                      BEGIN                                                                                                                                                                                                                                                                                                  UPDATE proveedores                                                                                                                                                                                                                                                                                 SET name       = p_name,                                                                                                                                                                                                                                                                           legal_name = p_legal_name,                                                                                                                                                                                                                                                                     tipo_id    = p_tipo_id,                                                                                                                                                                                                                                                                        tax_id     = p_tax_id,                                                                                                                                                                                                                                                                         email      = p_email,                                                                                                                                                                                                                                                                          phone      = p_phone,                                                                                                                                                                                                                                                                          address    = p_address,                                                                                                                                                                                                                                                                        notes      = p_notes,                                                                                                                                                                                                                                                                          is_active  = p_is_active,                                                                                                                                                                                                                                                                      updated_at = NOW()                                                                                                                                                                                                                                                                             WHERE id = p_id AND empresa_id = p_empresa_id AND deleted_at IS NULL;                                                                                                                                                                                                                              RETURN FOUND;                                                                                                                                                                                                                                                                                      END;                                                                                                                                                                                                                                                                                                   $$;
...
CREATE FUNCTION public.sp_reporte_documentos_estado(p_empresa_id bigint, p_fecha_desde date DEFAULT NULL::date, p_fecha_hasta date DEFAULT NULL::date) RETURNS TABLE(estado character varying, tipo_documento character varying, tipo_label character varying, cantidad bigint, total_comprobante numeric)
    LANGUAGE plpgsql
    AS $$                                                                                                                                                    BEGIN                                                                                                                                                RETURN QUERY                                                                                                                                     SELECT                                                                                                                                           d.estado,                                                                                                                                    d.tipo_documento,                                                                                                                            CASE d.tipo_documento                                                                                                                        WHEN '01' THEN 'Factura ElectrÃ³nica'                                                                                                     WHEN '02' THEN 'Nota de DÃ©bito'                                                                                                          WHEN '03' THEN 'Nota de CrÃ©dito'                                                                                                         WHEN '04' THEN 'Tiquete ElectrÃ³nico'                                                                                                     WHEN '08' THEN 'Factura de Compra'                                                                                                       WHEN '09' THEN 'Factura de ExportaciÃ³n'                                                                                                  WHEN '10' THEN 'Recibo ElectrÃ³nico de Pago'                                                                                              ELSE 'Otro (' || d.tipo_documento || ')'                                                                                                 END::VARCHAR                 AS tipo_label,                                                                                                  COUNT(*)::BIGINT             AS cantidad,                                                                                                    SUM(d.total_comprobante)     AS total_comprobante                                                                                            FROM documentos_electronicos d                                                                                                                   WHERE d.empresa_id  = p_empresa_id                                                                                                               AND d.deleted_at  IS NULL                                                                                                                      AND (p_fecha_desde IS NULL OR d.fecha_emision::DATE >= p_fecha_desde)                                                                          AND (p_fecha_hasta IS NULL OR d.fecha_emision::DATE <= p_fecha_hasta)                                                                          GROUP BY d.estado, d.tipo_documento                                                                                                              ORDER BY d.estado, d.tipo_documento;                                                                                                             END;                                                                                                                                                 $$;
...
CREATE FUNCTION public.sp_reporte_facturas_recibidas_resumen(p_empresa_id bigint, p_fecha_desde date DEFAULT NULL::date, p_fecha_hasta date DEFAULT NULL::date) RETURNS TABLE(estado_recepcion character varying, estado_hacienda character varying, cantidad bigint, total_comprobante numeric, total_impuesto numeric)
    LANGUAGE plpgsql
    AS $$                                                                                                                                                             BEGIN                                                                                                                                                         RETURN QUERY                                                                                                                                              SELECT                                                                                                                                                    f.estado_recepcion,                                                                                                                                   f.estado_hacienda,                                                                                                                                    COUNT(*)::BIGINT         AS cantidad,                                                                                                                 SUM(f.total_comprobante) AS total_comprobante,                                                                                                        SUM(f.total_impuesto)    AS total_impuesto                                                                                                            FROM facturas_recibidas f                                                                                                                                 WHERE f.empresa_id = p_empresa_id                                                                                                                         AND (p_fecha_desde IS NULL OR f.created_at::DATE >= p_fecha_desde)                                                                                      AND (p_fecha_hasta IS NULL OR f.created_at::DATE <= p_fecha_hasta)                                                                                      GROUP BY f.estado_recepcion, f.estado_hacienda                                                                                                            ORDER BY f.estado_recepcion;                                                                                                                              END;                                                                                                                                                          $$;
...
CREATE FUNCTION public.sp_reporte_inventario(p_empresa_id bigint, p_bodega_id bigint DEFAULT NULL::bigint, p_bajo_minimo boolean DEFAULT false, p_search text DEFAULT NULL::text) RETURNS TABLE(bodega_id bigint, bodega_nombre character varying, producto_id bigint, codigo character varying, nombre character varying, categoria character varying, unidad_medida character varying, precio numeric, stock numeric, stock_min numeric, bajo_minimo boolean)
    LANGUAGE plpgsql
    AS $$                                                                                                                                                                                                                                                                 BEGIN                                                                                                                                                                                                                                                             RETURN QUERY                                                                                                                                                                                                                                                  SELECT                                                                                                                                                                                                                                                        b.id                 AS bodega_id,                                                                                                                                                                                                                        b.name               AS bodega_nombre,                                                                                                                                                                                                                    p.id                 AS producto_id,                                                                                                                                                                                                                      p.code               AS codigo,                                                                                                                                                                                                                           p.name               AS nombre,                                                                                                                                                                                                                           COALESCE(c.name, '') AS categoria,                                                                                                                                                                                                                        COALESCE(p.unit_measure, '') AS unidad_medida,                                                                                                                                                                                                            COALESCE(p.price, 0) AS precio,                                                                                                                                                                                                                           COALESCE(bp.stock, 0) AS stock,                                                                                                                                                                                                                           COALESCE(bp.stock_min, 0) AS stock_min,                                                                                                                                                                                                                   (COALESCE(bp.stock, 0) < COALESCE(bp.stock_min, 0) AND bp.stock_min > 0) AS bajo_minimo                                                                                                                                                                   FROM bodegas b                                                                                                                                                                                                                                                JOIN bodega_productos bp ON bp.bodega_id = b.id                                                                                                                                                                                                               JOIN productos p         ON p.id = bp.producto_id                                                                                                                                                                                                             LEFT JOIN categorias_producto c ON c.id = p.categoria_id                                                                                                                                                                                                      WHERE b.empresa_id  = p_empresa_id                                                                                                                                                                                                                            AND p.empresa_id  = p_empresa_id                                                                                                                                                                                                                            AND p.type        = 'product'                                                                                                                                                                                                                               AND (p_bodega_id IS NULL OR b.id = p_bodega_id)                                                                                                                                                                                                             AND (NOT p_bajo_minimo OR (bp.stock < bp.stock_min AND bp.stock_min > 0))                                                                                                                                                                                   AND (                                                                                                                                                                                                                                                       p_search IS NULL OR p_search = ''                                                                                                                                                                                                                         OR p.name ILIKE '%' || p_search || '%'                                                                                                                                                                                                                    OR p.code ILIKE '%' || p_search || '%'                                                                                                                                                                                                                    )                                                                                                                                                                                                                                                           ORDER BY b.name, p.name;                                                                                                                                                                                                                                      END;                                                                                                                                                                                                                                                              $$;
...
CREATE FUNCTION public.sp_reporte_inventario_summary(p_empresa_id bigint) RETURNS TABLE(bodega_id bigint, bodega_nombre character varying, total_items bigint, bajo_minimo bigint, sin_stock bigint)
    LANGUAGE plpgsql
    AS $$                                                                                                              BEGIN                                                                                                          RETURN QUERY                                                                                               SELECT                                                                                                     b.id          AS bodega_id,                                                                            b.name        AS bodega_nombre,                                                                        COUNT(bp.id)  AS total_items,                                                                          COUNT(*) FILTER (WHERE bp.stock < bp.stock_min AND bp.stock_min > 0) AS bajo_minimo,                   COUNT(*) FILTER (WHERE bp.stock <= 0) AS sin_stock                                                     FROM bodegas b                                                                                             JOIN bodega_productos bp ON bp.bodega_id = b.id                                                            JOIN productos p         ON p.id = bp.producto_id                                                          AND p.empresa_id = p_empresa_id                                                     AND p.type = 'product'                                                              WHERE b.empresa_id = p_empresa_id                                                                          GROUP BY b.id, b.name                                                                                      ORDER BY b.name;                                                                                           END;                                                                                                           $$;
...
CREATE FUNCTION public.sp_reporte_libro_ventas(p_empresa_id bigint, p_fecha_desde date, p_fecha_hasta date, p_tipo_documento character varying DEFAULT NULL::character varying, p_estado character varying DEFAULT NULL::character varying) RETURNS TABLE(id bigint, fecha_emision timestamp without time zone, tipo_documento character varying, tipo_label character varying, numero_consecutivo character varying, clave character varying, receptor_nombre character varying, receptor_numero_id character varying, moneda character varying, total_venta_neta numeric, total_impuesto numeric, total_comprobante numeric, estado character varying)
    LANGUAGE plpgsql
    AS $$                                                                                                                                                                                                                                                                                                                                                                                                BEGIN                                                                                                                                                                                                                                                                                                                                                                                            RETURN QUERY                                                                                                                                                                                                                                                                                                                                                                                 SELECT                                                                                                                                                                                                                                                                                                                                                                                       d.id,                                                                                                                                                                                                                                                                                                                                                                                    d.fecha_emision,                                                                                                                                                                                                                                                                                                                                                                         d.tipo_documento,                                                                                                                                                                                                                                                                                                                                                                        CASE d.tipo_documento                                                                                                                                                                                                                                                                                                                                                                    WHEN '01' THEN 'Factura ElectrÃ³nica'                                                                                                                                                                                                                                                                                                                                                 WHEN '02' THEN 'Nota de DÃ©bito'                                                                                                                                                                                                                                                                                                                                                      WHEN '03' THEN 'Nota de CrÃ©dito'                                                                                                                                                                                                                                                                                                                                                     WHEN '04' THEN 'Tiquete ElectrÃ³nico'                                                                                                                                                                                                                                                                                                                                                 WHEN '08' THEN 'Factura de Compra'                                                                                                                                                                                                                                                                                                                                                   WHEN '09' THEN 'Factura de ExportaciÃ³n'                                                                                                                                                                                                                                                                                                                                              WHEN '10' THEN 'Recibo ElectrÃ³nico de Pago'                                                                                                                                                                                                                                                                                                                                          ELSE 'Otro (' || d.tipo_documento || ')'                                                                                                                                                                                                                                                                                                                                             END::VARCHAR                                               AS tipo_label,                                                                                                                                                                                                                                                                                                                d.numero_consecutivo,                                                                                                                                                                                                                                                                                                                                                                    d.clave,                                                                                                                                                                                                                                                                                                                                                                                 COALESCE(d.receptor_nombre,    'Consumidor Final')::VARCHAR AS receptor_nombre,                                                                                                                                                                                                                                                                                                          COALESCE(d.receptor_numero_id, 'â€”')::VARCHAR                AS receptor_numero_id,                                                                                                                                                                                                                                                                                                       d.moneda,                                                                                                                                                                                                                                                                                                                                                                                d.total_venta_neta,                                                                                                                                                                                                                                                                                                                                                                      d.total_impuesto,                                                                                                                                                                                                                                                                                                                                                                        d.total_comprobante,                                                                                                                                                                                                                                                                                                                                                                     d.estado                                                                                                                                                                                                                                                                                                                                                                                 FROM documentos_electronicos d                                                                                                                                                                                                                                                                                                                                                               WHERE d.empresa_id  = p_empresa_id                                                                                                                                                                                                                                                                                                                                                           AND d.deleted_at  IS NULL                                                                                                                                                                                                                                                                                                                                                                  AND d.fecha_emision::DATE BETWEEN p_fecha_desde AND p_fecha_hasta                                                                                                                                                                                                                                                                                                                          AND (p_tipo_documento IS NULL OR d.tipo_documento = p_tipo_documento)                                                                                                                                                                                                                                                                                                                      AND (p_estado         IS NULL OR d.estado         = p_estado)                                                                                                                                                                                                                                                                                                                              ORDER BY d.fecha_emision DESC;                                                                                                                                                                                                                                                                                                                                                               END;                                                                                                                                                                                                                                                                                                                                                                                             $$;
...
CREATE FUNCTION public.sp_reporte_ranking_clientes(p_empresa_id bigint, p_fecha_desde date, p_fecha_hasta date, p_limit integer DEFAULT 20) RETURNS TABLE(posicion bigint, receptor_nombre character varying, receptor_numero_id character varying, cantidad_docs bigint, total_venta_neta numeric, total_impuesto numeric, total_comprobante numeric)
    LANGUAGE plpgsql
    AS $$                                                                                                                                                                                              BEGIN                                                                                                                                                                                          RETURN QUERY                                                                                                                                                                               SELECT                                                                                                                                                                                     ROW_NUMBER() OVER (ORDER BY SUM(d.total_comprobante) DESC)::BIGINT AS posicion,                                                                                                        COALESCE(d.receptor_nombre,    'Consumidor Final')::VARCHAR AS receptor_nombre,                                                                                                        COALESCE(d.receptor_numero_id, 'â€”')::VARCHAR                AS receptor_numero_id,                                                                                                     COUNT(*)::BIGINT              AS cantidad_docs,                                                                                                                                        SUM(d.total_venta_neta)       AS total_venta_neta,                                                                                                                                     SUM(d.total_impuesto)         AS total_impuesto,                                                                                                                                       SUM(d.total_comprobante)      AS total_comprobante                                                                                                                                     FROM documentos_electronicos d                                                                                                                                                             WHERE d.empresa_id  = p_empresa_id                                                                                                                                                         AND d.estado      = 'aceptado'                                                                                                                                                           AND d.deleted_at  IS NULL                                                                                                                                                                AND d.fecha_emision::DATE BETWEEN p_fecha_desde AND p_fecha_hasta                                                                                                                        GROUP BY d.receptor_nombre, d.receptor_numero_id                                                                                                                                           ORDER BY SUM(d.total_comprobante) DESC                                                                                                                                                     LIMIT p_limit;                                                                                                                                                                             END;                                                                                                                                                                                           $$;
...
CREATE FUNCTION public.sp_reporte_ventas_periodo(p_empresa_id bigint, p_fecha_desde date, p_fecha_hasta date) RETURNS TABLE(tipo_documento character varying, tipo_label character varying, cantidad bigint, total_venta_neta numeric, total_impuesto numeric, total_comprobante numeric)
    LANGUAGE plpgsql
    AS $$                                                                                                                                                               BEGIN                                                                                                                                                           RETURN QUERY                                                                                                                                                SELECT                                                                                                                                                      d.tipo_documento,                                                                                                                                       CASE d.tipo_documento                                                                                                                                   WHEN '01' THEN 'Factura ElectrÃ³nica'                                                                                                                WHEN '02' THEN 'Nota de DÃ©bito'                                                                                                                     WHEN '03' THEN 'Nota de CrÃ©dito'                                                                                                                    WHEN '04' THEN 'Tiquete ElectrÃ³nico'                                                                                                                WHEN '08' THEN 'Factura de Compra'                                                                                                                  WHEN '09' THEN 'Factura de ExportaciÃ³n'                                                                                                             WHEN '10' THEN 'Recibo ElectrÃ³nico de Pago'                                                                                                         ELSE 'Otro (' || d.tipo_documento || ')'                                                                                                            END::VARCHAR                  AS tipo_label,                                                                                                            COUNT(*)::BIGINT              AS cantidad,                                                                                                              SUM(d.total_venta_neta)       AS total_venta_neta,                                                                                                      SUM(d.total_impuesto)         AS total_impuesto,                                                                                                        SUM(d.total_comprobante)      AS total_comprobante                                                                                                      FROM documentos_electronicos d                                                                                                                              WHERE d.empresa_id  = p_empresa_id                                                                                                                          AND d.estado      = 'aceptado'                                                                                                                            AND d.deleted_at  IS NULL                                                                                                                                 AND d.fecha_emision::DATE BETWEEN p_fecha_desde AND p_fecha_hasta                                                                                         GROUP BY d.tipo_documento                                                                                                                                   ORDER BY SUM(d.total_comprobante) DESC;                                                                                                                     END;                                                                                                                                                            $$;
